...
export const TasksApi = {
  ...
  getTaskDetails: {
    ...
    fn: async (id: string): Promise<TGetTaskDetailsDto> => {
      const { data } = await apiClient.get<TGetTaskDetailsDto>(`/tasks/${id}/details`);
      return data;
    },
  },
  ...
  createTask: {
    ...
    fn: async (task: TCreateTaskReqDto): Promise<TCreateTaskResponse> => {
      const { data } = await apiClient.post<TCreateTaskResponse>("/tasks", task);
      return data;
    },
  },
  ...
  updateTask: {
    ...
    fn: async ({ id, ...task }: TUpdateTaskDto): Promise<TUpdateTaskResponse> => {
      const { data } = await apiClient.patch<TUpdateTaskResponse>(`/tasks/${id}`, task);
      return data;
    },
  },
  ...
  submitProgress: {
    ...
    fn: async (progress: TSubmitProgressDto): Promise<TSubmitProgressResponse> => {
      const { data } = await apiClient.post<TSubmitProgressResponse>("/progresses", progress);
      return data;
    },
  },
